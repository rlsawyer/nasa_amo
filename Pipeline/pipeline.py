    ...
    def blend_lines_into_image(self, thr_img):
        ...
        for p in prps:
            ...
            for i in range(st_idx - tmp_bfr, st_idx + np.round((fn_idx - st_idx) / 2)):
                ns_i = (np.random.randn(max_r + 2 - min_r) - 0.0) / 100
                ns_fn_i = (np.random.randn(max_r + 2 - min_r) - 0.0) / 100
                ...
                if np.sum(tmp_i > 0) < (col_thr * img.shape[1]):
                    inp_i = inp_fn_i
                if np.sum(tmp_fn_i > 0) < (col_thr * img.shape[1]):
                    inp_fn_i = inp_i
            ...
            max_c = np.min([final_max_c + bfr + 1, init_img.shape[1]])
