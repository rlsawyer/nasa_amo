...
class InformationRetreival(object):
    ...
    def extract_frames_test_video(self, filename, path, nframes):

        save_path = join(path, 'frames')
    ...
    def get_directories(self, type):
        ...
        if type == 'bmode':
            source_dir = self.processedBmode
    ...
    def partition_and_store(self, sheet, type):
        ...
        if type == 'bmode':
            start, end = 414, 846
            TEST_NEG, TEST_POS, TRAIN_NEG, TRAIN_POS = 29*17, 52*10, 120*17, 203*10
        else:
            start, end = 2, 413
...
class Preprocess():
    ...
    def clean_images(self, home_dir='', save_dir=''):
        ...
        for name_img in list_imgs:
            if img_idx < n_imgs:
                if '.bmp' not in name_img and '.jpg' not in name_img and '.jpg' not in name_img:
                    print(name_img)
                    continue
                ...
                imageio.imwrite(crpd_final_img[0:len(crpd_final_img) - 4] + '.jpg', self.CleanedImage)
                img_idx += 1

    def preprocess_test_image(self, filename, path, mode):

        self.Mode = mode
        path_img = join(path, filename)
        c_img = Image.open(path_img)
        pil_imgray = c_img.convert('LA')
        c_img = np.asarray(c_img)
        img = np.array(list(pil_imgray.getdata(band=0)), float)
        ...
        name_img = filename[0:len(filename)-4] + '_processed.jpg'
        save_path = join(path, 'processed')
        if not isdir(save_path):
                mkdir(save_path)
        crpd_final_img = join(save_path, name_img)
        self.CleanedImage = cleaned_img
        self.remove_text()
...
class DataAugmentation(object):
    ...
    def train_val_split(self, type, percent):
        ...
        if type == 'mmode':
            ...
            sfiles = glob.glob(os.path.join(source_neg, '*.jpg'))
        ...
        if type == 'mmode':
            ...
            sfiles = glob.glob(os.path.join(source_pos, '*.jpg'))
    ...
    def affine_transform_bmode(self, type):
        ...
        # TODO: Make this snippet into util function
...
def main():
    ...
    #info.processedBmode = os.path.join(info.ptx, 'bmode_images')
